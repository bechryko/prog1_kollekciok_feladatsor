...
public class Main {
    public static void main(String[] args) {
        /* tipp a teszteléshez: hívd meg a saját függvényeidet az enyéim mellett a példa inputokra, és hasonlítsd össze,
        hogy mit adnak vissza */
        koviFeladat();
        ...
        String[] megold = feladat11(
            ...
            new String[] {"narancs", "fekete", "fehér"}
        );
    ...
    public static List<Long> feladat1a() {
        /* a listákat fel lehet fogni olyan leképezésekként, amik az első N természetes számhoz egy tetszőleges
        halmazból rendelnek elemeket (ez a tetszőleges halmaz jelenleg a Long-ok, hiszen a 4910752992 nem férne bele
        egy Integer típusú változóba) */
        ...
        // (leegyszerűsítve)
        /* elég úgy hozzárendelni az első halmaz elemeihez a második elemeit, hogy semelyik kettőhöz sem rendeljük
        ugyanazt (mivel a második halmazban több elem van, mint az elsőben, a szürjektivitás könnyen teljesül) */
        return new ArrayList<>(Arrays.asList(3L, 5L, 10L, 625L, 4910752992L));
        /* a feladat megoldható tömbökkel is, hiszen ott is az első N természetes szám az indexek halmaza (azaz az
        indulási halmaz),de a feladatsor a kollekciókról szól, úgyhogy inkább gondolkozzunk azokban, hacsak a feladat
        mást nem mond */
    }
    ...
    public static Map<Integer, String> feladat1b() {
        /* a Map-eket (leképezéseket, asszociatív tömböket) is (nem túl meglepő módon) el lehet képzelni dimat1-ről
        ismerős leképezésekként, csakhogy ezek már tetszőleges halmaz (jelen esetben: Integerek) elemeihez tetszőleges
        halmaz (jelen esetben: Stringek) elemeit tudják rendelni */
        ...
        /* természetesen az előző feladatot is meg lehet csinálni Map segítségével, viszont ha az első N természetes
        számhoz kell rendelnünk, akkor a lista is tökéletesen megfelel */
        /* ennek a feladatnak a megoldása nem jó listával, mert az az 1, 3, 5 és 7 értékekhez is rendelne valamilyen
        (null) értéket, az pedig más leképezés lenne, mint amit a feladat kér */
    }
    ...
    public static List<?>[] feladat2(List<String> szurnivalo) {
        ...
        /* általános tipp: ha egy változó típusa nem egy konkrét osztály, hanem egy absztrakt osztály vagy interface
        (esetünkben List), akkor a konkrét osztályok példányait is tárolhatja (ArrayList, LinkedList, Vector, Stack),
        érdemes ezt használni */
        /* ez a tanács akkor is érvényes, amikor hagyományos osztályokról van szó, amik között öröklődés van: ekkor is
        érdemesebb az ősosztályt használni, amennyiben rendelkezik azokkal a funkciókkal, amikre szükségünk van */
        List<Boolean> igazsagertekek = new ArrayList<>();
        List<Long> szamok = new ArrayList<>();
        List<String> szovegek = new ArrayList<>();
        /* for-each ciklussal végignézzük az elemeket (nagyon hasznos, ha csak olvasni akarjuk a kollekció elemeit,
        nem írni vagy törölni) */
        for (String str: szurnivalo) {
            // boolean értékek hozzáadása, egyszerű equals metódusokkal
            /* azért a szöveg literálnak (konkrét szövegnek) hívom meg az equals metódusát, nem az str-nek, mert ha az
            null, akkor exception-t kapunk */
            ...
            akkor kapunk egy NumberFormatException-t (ki lehet puskázni a parseLong metódus leírásából), amit elkapunk,
            és abban az esetben szövegként tároljuk el az eredményt */
            ...
            /* úgy is meg lehet csinálni a számok felismerését, hogy karakterenként végigmegyünk a szövegen, és
            megnézzük, hogy minden karaktere szám-e */
        }
        return new List<?>[]{igazsagertekek, szamok, szovegek}; // List[]{...} is tökéletes
        /* itt látunk valami izgalmasat (a szemfüles a fejlécben is láthatott): a generikus típusnál ?-t (ún.
        wildcard-ot) használuk, ha nem tudjuk, hogy milyen típusú lesz a lista
        alapvetően egy tömbben csak egyféle értéket tárolhatunk, és bár mindhárom értékünk lista, nem ugyanolyan listák,
        ezért nem rakhatjuk őket pl egy List<Integer>[]-be
        használhatjuk ez esetben a raw lista típust, az is működik, de a wildcard-okkal való megvalósítás sokkal szebb,
        és az IDEA sem panaszkodik miatta */
        /* a wildcard-ok nem részei a prog1 gyakorlati anyagnak, de akit érdekel, a következő linken bővebben utánuk
        olvashat: https://okt.inf.szte.hu/alkfejl1/gyakorlat/misc/generics/#wildcards */
    }
    ...
    public static List<Integer> feladat3(List<Integer> lista) {
        ...
        /* for ciklusokkal is elvégezhetnénk a törlést és a duplázást (kivéve for-each ciklussal!!!), de a removeIf és a
        replaceAll metódusokkal sokkal egyszerűbb és gyorsabb (és szebb is tőlük a kód) */
        ...
        return ujLista;
        /* ha nem hoztunk létre új listát, hanem a paraméterben kapottat módosítottuk, az rossz megoldás! (nem az volt a
        feladat) */
    }

    /**
     * Elméleti feladat: hány különböző igazságértékeket tartalmazó halmazt kell létrehozni (és valahogyan feltölteni
     * elemekkel), hogy biztosan legyen köztük kettő egyenlő?
     */
    public static void feladat4() {
        /* egy igazságértékeket tároló halmaz, mivel kollekció, nem primitív értékeket, hanem referencia típusúakat tud
        tárolni: ezért Boolean halmazt kell létrehoznunk */
        // a Boolean típusú változónak 3 féle értéke lehet: true, false és null (null is, mivel referencia típusú)
        /* mind a három féle értéket vagy bevehetjük a halmazba, vagy nem (mivel a sorrendiség nem számít, csak hogy
        tartalmazza-e) ez 2 lehetőség minden értékre, összesen 2^3=8 lehetőség */
        // a következő kódrészlet ezt szimulálja: ha a megfelelő változó értéke 0, nem vesszük bele, ha 1, akkor igen
        ...
        /* ez a 8 féle Boolean Set létezik, ha bármilyen más halmazt létrehozunk, akkor biztosan egyenlő lesz
        valamelyikkel */
    ...
    public static int feladat6(Map<String, String> map) {
        ...
        /* a másik halmaz elemeit az addAll metódussal adhatjuk hozzá a halmazhoz (ez is bármilyen kollekciót kaphat
        paraméterben) */
        szavak.addAll(map.values());
        /* ahogy a listáknál is, a halmazoknál is van egy removeIf metódus, amivel a feltételnek megfelelő elemeket
        eltávolíthatjuk */
     ...
     * listájával, akik megbuknak, azaz legalább egy tantárgyból 1-est szereztek!
    ...
    public static List<String> feladat7(Map<String, Map<String, Integer>> diakok) {
        ...
        for (Map.Entry<String, Map<String, Integer>> diak : diakok.entrySet()) {
            ...
            for (Map.Entry<String, Integer> tantargy : diak.getValue().entrySet()) {
                if (tantargy.getValue() == 1) {
                    ...
                    /* break-elnünk, mert nem akarunk duplikált neveket abban az esetben, ha valaki több tárgyból is
                    bukik */
                    break;
                }
            }
        ...
        /* még kétféle módon nézhettük volna végig a Map-eket: iterátorral, vagy olyan foreach ciklussal, ami a Map-ek
        keySet-jén iterál végig, és így a kulcs alapján kérjük le az értéket */
        // rendezzük a nevek listáját abc sorrendben
        /* ha null-t írunk, az az adott adattípuson értelmezett alapvető rendezés (Stringeknél abc sorrend, saját
        típusnál nekünk kellene megadnunk a Comparable interface segítségével) */
        bukottDiakok.sort(null);
        // a következő megoldás ugyanezt csinálja:
        //bukottDiakok.sort(Comparator.naturalOrder());
        /* meg is adhatjuk azt a metódust, amivel szeretnénk elvégezni két elem összehasonlítását a rendezéshez; ez a
        String osztályban lévő compareTo metódus */
     ...
     * Amíg Karcsi nem figyelt, ellopta a leképezését, és most ki akarja törölni azokat a kulcs-érték párokat, amikre a
     * fenti szabály igaz.
     * De sietnie kell! Valósítsd meg a fenti működést mindössze egyetlen sorban! (1 sor vagy 1 egysoros ciklus lehet
     * benne.)
    ...
    public static void feladat8(Map<Integer, Integer> lekepezes) {
        ...
        /*for(int i = Integer.MIN_VALUE / 3; i <= Integer.MAX_VALUE / 3; i++)
            /* érdemes megjegyezni, hogy a Map-ek remove metódusa tud konkrét kulcs-érték párt is törölni, és amennyiben
            nincs ilyen kulcs-érték páros, nem csinál semmit */
            //lekepezes.remove(i, i * 3);
        /* a második megoldás sokkal kifinomultabb (és látványosan gyorsabb, érdemes kipróbálni!), ez a Map-ek removeIf
        metódusát használja (itt a lambda-kifejezésünkben e egy entry) */
        lekepezes.entrySet().removeIf(e -> e.getKey() * 3 == e.getValue());
    }

    /**
     * Vivi hónapok óta feljegyzi, ami vele történt, most azonban megelégelte a sok papírt, és össze akarja gyűjteni a
     * feljegyzéseket egyetlen naplóba.
     * Egy feljegyzést egyértelműen beazonosít egy dátum (szöveg), és tartozik még hozzá maga a feljegyzés szövege.
     * Vivi azonban nem akarja beleírni az összes feljegyzést a naplójába. Vannak köztük kínosak és nagyon rövidek,
     * amiket felesleges átírnia.
     * Írd át a feljegyzések listáját a naplóba, de csak olyan feljegyzéseket, amelyeknek a szövege több mint 5 szó
     * hosszú, és nem tartalmazza a "kínos", "szerelmes" és "anime" szavak egyikét sem.
    ...
    public static Map<String, String> feladat9(List<Map.Entry<String, String>> feljegyzesek) {
        ...
        for (Map.Entry<String, String> feljegyzes: feljegyzesek) {
            /* minden feljegyzés szövegére megnézzük, hogy a szavak száma nagyobb-e 5-nél (feldaraboljuk a szöveget
            szóközök mentén, és ennek a tömbnek az elemszáma a szavak száma), majd megnézzük, hogy az eredeti szöveg
            tartalmazza-e valamelyik szót a megadottak közül, és ha nem, hozzáadjuk a feljegyzést a Map-hez */
            if (
                feljegyzes.getValue().split(" ").length > 5 &&
                !feljegyzes.getValue().contains("kínos") &&
                !feljegyzes.getValue().contains("szerelmes") &&
                !feljegyzes.getValue().contains("anime")
            ) {
                naplo.put(feljegyzes.getKey(), feljegyzes.getValue());
                /* másik megoldásnak gondolhatjuk a következő sort, ahol közvetlenül a Map Entry Set-jéhez adjuk hozzá a
                feljegyzést: */
                //naplo.entrySet().add(feljegyzes);
                /* ez nem fog működni, az entrySet metódus leírását olvasva a következő sort láthatjuk:
                "It does not support the add or addAll operations."
                ha egy olyan metódust használunk, aminek nem ismerjük pontosan a működését, érdemes átfutni a leírást,
                különben kellemetlen meglepetésekbe (pl. itt: UnsupportedOperationException) futhatunk */
            }
        }
        return naplo;
    }
     ...
     * Akkor tartozzon két halmaz egy osztályba, ha az elemeik szorzatának előjele ugyanaz. (-1, 0, 1, ez legyen a három
     * osztály neve is.)
     * Készítsd el a HalmazOsztalyozas osztályt, egy függvényt, ami visszaad egy olyan Map-et, amire a
     * map.get(new HalmazOsztalyozas(halmaz))
    ...
    public static Map<HalmazOsztalyozas, Integer> feladat10() {
        ...
        /* a Map-hez egyszerűen hozzá kell adnunk három kulcs-érték párt, ahol a kulcs egy HalmazOsztalyozas
        (paraméterben egy példa halmazzal az adott osztályban), az érték pedig az osztály */
     ...
     * Írj egy függvényt, ami tetszőlegesen sok szövegtömböt vár paraméterként. Ezekben a tömbökben az adott napon
     * kimosott pólók színei vannak.
    ...
    public static String[] feladat11(String[]... napok) {
        ...
        for(int nap = 1; !szekreny.isEmpty(); nap++) {
            ...
            if(nap < napok.length && napok[nap] != null) {
                /* tömb elemeit nem tudjuk hozzáadni a Stackhez, csak egy unalmas for ciklus segítségével, viszont ha
                előtte átalakítjuk listává, akkor már használhatjuk az addAll metódust */
                szekreny.addAll(Arrays.asList(napok[nap]));
            }
        }
        /* a listát át tudjuk alakítani tömbbé (mert a feladat tömbben kérte a végeredmény) a List interface toArray
        metódusával
        ...
        // természetesen a Stack használata nélkül is megoldható a feladat, de talán így a legkézenfekvőbb
    }

    /**
     * Jánoska játékokat szeretne Karácsonyra. Sokat. Éppen ezért levelet ír a Jézuskának, arról, hogy milyen játékokat
     * kér.
     * Használd fel az előre elkészített Jatek osztályt, és egészítsd ki a megfelelő módon!
     * Írj egy függvényt, ami várja Jánoska levelét (egy listát a játékokról), és egy egész számot, hogy hány játékot
     * tud megvenni neki a Jézuska.
    ...
    public static Set<String> feladat12(List<Jatek> level, int n) {
        ...
        for (int i = 0; i < n; i++) {
            /* a peek metódussal meg tudjuk nézni a sor következő elemét, és ha az null (azaz elfogytak a játékok),
            akkor kilépünk a ciklusból */
            if(jatekok.peek() == null) {
                break;
            }
            // különben hozzáadjuk a megvásárolt játékokhoz a következő játék nevét
            /* a poll metódus nem csak visszatér a sorban következő elemmel, hanem ki is veszi a sorból, így több
            dolgunk nincs vele */
            megvasaroltJatekok.add(jatekok.poll().getNev());
        }
        return megvasaroltJatekok;
        /* természetesen máshogyan is meg lehet oldani a feladatot, de az érdekesség kedvéért egy órán nem tárgyalt
        kollekcióval, a prioritási sorral mutattam meg */
    }
